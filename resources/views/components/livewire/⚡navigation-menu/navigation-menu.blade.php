<nav
    x-data="{ open: false }"
    class="border-b border-gray-100 bg-neutral-200 text-neutral-600 dark:bg-neutral-700 dark:text-neutral-200"
>
    @php $user = auth()->user(); @endphp

    {{-- primary navigation menu --}}
    <div class="flex min-h-16 gap-5 px-2">
        <div class="flex grow gap-5">
            {{-- logo --}}
            <div class="flex shrink-0 items-center">
                <a href="{{ route('home') }}" title="{{ __('app.home') }}">
                    <x-svg.genealogy
                        class="hover:fill-primary-300 dark:hover:fill-primary-300 size-12 dark:fill-neutral-400"
                        alt="genealogy"
                    />
                </a>
            </div>
            ...
            <div class="flex flex-wrap gap-5 py-2">
                <div class="flex items-center gap-5">
                    <x-nav-link href="{{ route('home') }}" :active="request()->routeIs('home')">
                        <x-ts-icon icon="home" class="mr-1 inline-block size-5" />
                        {{ __('app.home') }}
                    </x-nav-link>
...
{{--                    </x-nav-link>--}}
                ...
                <div class="flex items-center gap-5">
                    <x-nav-link href="{{ route('people.birthdays') }}" :active="request()->routeIs('people.birthdays')">
                        <x-ts-icon icon="tabler.cake" class="mr-1 inline-block size-5" />
            ...
            @auth
                ...
                <div class="relative min-w-max">
                    <x-dropdown align="left" width="48">
                        ...
                        <x-slot name="content">
                            {{-- account management --}}
                            <div class="block px-4 py-2 text-xs text-gray-400">{{ __('app.manage_account') }}</div>
                            ...
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                ...
                                <x-dropdown-link href="{{ route('logout') }}" @click.prevent="$root.submit()">
                                    <x-ts-icon icon="tabler.logout" class="mr-1 inline-block size-5" />
                                    {{ __('auth.logout') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
...
{{--                @endif--}}
        ...
        <div class="flex flex-col place-items-center">
            <div class="hidden min-h-8 space-x-6 md:ml-5 md:flex md:items-center">
        ...
        <div class="flex items-center md:hidden">
            <button
                @click="open = ! open"
                class="inline-flex items-center justify-center rounded-sm p-2 text-gray-400 transition duration-150 ease-in-out hover:bg-gray-300 hover:text-gray-500 focus:bg-gray-300 focus:text-gray-500 focus:outline-hidden"
            >
...
{{--            </x-nav-link-responsive>--}}
            ...
            <x-nav-link-responsive
                ...
                :active="request()->routeIs('people.birthdays')"
            >
                <x-ts-icon icon="tabler.cake" class="mr-1 inline-block size-5" />
                {{ __('birthday.birthdays') }}
            </x-nav-link-responsive>
...
{{--                @endif--}}
            ...
            <div class="border-t border-gray-200 pt-2 pb-2 pl-4">
                <x-set.language />
            </div>
            <div class="border-t border-gray-200 pt-2 pb-2 pl-4">
                <x-ts-theme-switch simple only-icons />
...
{{--                        @endcan--}}
            ...
            {{-- responsive settings options --}}
            <div class="border-t border-gray-200 py-2 pl-4">
                <x-ts-theme-switch simple only-icons />
            </div>
            <div class="border-t border-gray-200 py-1 pl-4">
                <x-set.offcanvas />
            </div>
        @endauth
    </div>
