<div
    class="flex max-w-3xl min-w-sm flex-col rounded-sm bg-white text-neutral-800 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 dark:text-neutral-50">
    <div
            ...
            @if (auth()->user()->hasPermission('person:update') || auth()->user()->hasPermission('person:delete'))
                <div class="max-w-min min-w-max flex-1 grow text-end">
                    <x-ts-dropdown icon="tabler.menu-2" position="bottom-end">
                        ...
                        @if ($person->isDeletable() && auth()->user()->hasPermission('person:delete'))
                            <hr/>
    ...
    {{-- lifetime & age --}}
    <div class="flex px-2">
        <div class="grow">{!! $person->lifetime ?? '&nbsp;' !!}</div>
    ...
    <div class="p-2">
        <table class="w-full">
            ...
            @if ($person->isDeceased())
                ...
                @if($person->getMetadataValue('cemetery_location_name'))
                    <tr class="align-top">
                ...
                @endif
                @if($person->getMetadataValue('cemetery_location_address'))
                    <tr class="align-top">
