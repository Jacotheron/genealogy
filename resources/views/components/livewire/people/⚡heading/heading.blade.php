<div class="rounded-sm bg-white p-2 pb-0 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700 dark:text-neutral-200">
        ...
        <div class="print:hidden">
            <div class="flex flex-wrap gap-1">
                <x-ts-button
                    ...
                    color="{{ request()->routeIs('people.show') ? 'yellow' : 'primary' }}"
                    class="mb-2 text-xs"
                    title="{{ __('person.profile') }}"
                >
                    <x-ts-icon icon="tabler.id" class="inline-block size-4" />
                </x-ts-button>

                <x-ts-button
                    href="/people/{{ $person->id }}/ancestors"
                    color="{{ request()->routeIs('people.ancestors') ? 'yellow' : 'secondary' }}"
                    class="mb-2 text-xs"
                    title="{{ __('person.ancestors') }}"
                >
                    <x-ts-icon icon="tabler.binary-tree" class="inline-block size-4 rotate-180" />
                </x-ts-button>

                <x-ts-button
                    href="/people/{{ $person->id }}/descendants"
                    color="{{ request()->routeIs('people.descendants') ? 'yellow' : 'secondary' }}"
                    class="mb-2 text-xs"
                    title="{{ __('person.descendants') }}"
                >
                ...
                <x-ts-button
                    ...
                    color="{{ request()->routeIs('people.chart') ? 'yellow' : 'secondary' }}"
                    class="mb-2 text-xs"
                    title="{{ __('app.family_chart') }}"
                >
                    <x-ts-icon icon="tabler.social" class="inline-block size-4" />
                </x-ts-button>

                <x-ts-button
                    href="/people/{{ $person->id }}/timeline"
                    color="{{ request()->routeIs('people.timeline') ? 'yellow' : 'secondary' }}"
                    class="mb-2 text-xs"
