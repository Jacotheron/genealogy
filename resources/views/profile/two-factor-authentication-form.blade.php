<x-action-section>
    <x-slot name="title">
        <div class="dark:text-gray-400">{{ __('user.2fa') }}</div>
    </x-slot>

    <x-slot name="description">
        <div class="dark:text-gray-100">{{ __('user.2fa_add') }}</div>
    </x-slot>
    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                <div class="mt-4 inline-block bg-white p-2">{!! $this->user->twoFactorQrCodeSvg() !!}</div>
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">{{ __('user.2fa_store_codes') }}</p>
                </div>
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-confirms-password wire:then="regenerateRecoveryCodes">
                        <x-ts-button color="secondary" class="me-3"> {{ __('user.2fa_regenerate') }} </x-ts-button>
                    </x-confirms-password>
                ...
                @else
                    <x-confirms-password wire:then="showRecoveryCodes">
                        <x-ts-button color="secondary" class="me-3"> {{ __('user.2fa_show') }} </x-ts-button>
                    </x-confirms-password>
                @endif
