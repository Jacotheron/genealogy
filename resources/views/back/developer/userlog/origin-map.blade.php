...
<x-app-layout>
    ...
    <script>
        const map = new svgMap({
            countryNames: @json($countries, JSON_THROW_ON_ERROR),
            targetElementID: 'svgMap',
            flagType: 'emoji',
            noDataText: @json($nodata, JSON_THROW_ON_ERROR),
            data: {
                data: {
                    visitors: {
                        name: @json($title, JSON_THROW_ON_ERROR),
                        format: '{0}',
                        thousandSeparator: ',',
                    },
                },
                applyData: 'visitors',
                values: @json($data, JSON_THROW_ON_ERROR),
            },
        });
    </script>
</x-app-layout>
